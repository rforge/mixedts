...
# distribution developed in [Rroji and Mercuri 2014]
  ...
  if(MixingDens=="User"){
    invFFT<-ChFunToDens.User(n=N,alim=alim,blim=blim,
                             ...
                             lambda_m=lambda_m,UseMGF=UseMGF,paramMixing=paramMixing)
    dens<-approx(invFFT$x,invFFT$density,x)
    return(dens$y)
  }
}
...
pMixedTS.aux <- function(q,mu0,mu,sig,a,alpha,lambda_p,lambda_m,lower=-Inf,
 ...
 # if(MixingDens=="Gamma"){ 
          myfun<- function(x,env){
            ...
            env$N->N1
            n<-length(x)
            y<-dMixedTS.aux(x,mu0=mu0,mu=mu,sig=sig,a=a,alpha=alpha,lambda_p=lambda_p,
                       lambda_m=lambda_m,setSup=setSup,setInf=setInf,MixingDens=MixingDens,
                       N=N1,UseMGF=UseMGF, paramMixing=paramMixing)
   }
    lower<--Inf
      if(!is.finite(lower)){
...
qMixedTS.aux <- function(p,mu0,mu,sig,a,
  ...
  myObjectFun<-function(q,prob,env){
    ...
    y <- pMixedTS.aux(q=q,mu0=mu0,mu=mu,sig=sig,a=a,alpha=alpha,
                  ...
                  UseMGF=UseMGF,paramMixing=paramMixing,N=N1)-prob
  }
  if(length(p)==1){
...
#    res <- fsolve(f=myObjectFun,x0=0.1,prob=p,env=env)$x
    ...
    probabilityTS<-pMixedTS.aux(stepquant,mu0,mu,sig,a,alpha,lambda_p,lambda_m,lower=-Inf,
                         ...
                         UseMGF=UseMGF,paramMixing=paramMixing,N=env$N)
...
rMixedTS.aux <- function(n,mu0,mu,sig,a,alpha,lambda_p,lambda_m,setSup=NULL,setInf=NULL,MixingDens="Gamma",UseMGF, paramMixing, Nstep=2^12){
  UniformNumber  <- runif(n, min = 0.0001, max = 0.999)
  MixedTSNumber  <- qMixedTS.aux(UniformNumber,mu0,mu,sig,a,alpha,lambda_p,lambda_m,setSup=setSup,setInf=setInf,
                                 MixingDens=MixingDens,interval=c(-100,100),
                                 UseMGF=UseMGF, paramMixing=paramMixing,N=Nstep)
